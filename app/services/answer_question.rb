class AnswerQuestion < ApplicationService
  ...
  def initialize(question:, relevant_pages:)
    @question = question
    @relevant_pages = relevant_pages
  end

  def call
    prompt = format(PROMPT, question: @question, context: context)
    Rails.logger.debug(prompt)

    OpenAIClient.fetch_completions(prompt)
  end
  ...
  private
    ...
    def context
      ...
      @relevant_pages.each do |page|
        chosen_sections_len += page.tokens + SEPARATOR_LEN

        if chosen_sections_len > MAX_SECTION_LEN
          space_left = MAX_SECTION_LEN - chosen_sections_len - SEPARATOR_LEN
          chosen_sections << SEPARATOR + page.content[0..space_left]
          break
        end

        chosen_sections << SEPARATOR + page.content
      end
